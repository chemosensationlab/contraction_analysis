# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 20:14:21 2023
...
# Neue Excel-Datei erstellen
    ...
    for m in names:
        ...
        worksheet.cell(row=1, column=start_col).value = 'duration '+str(m)
        #worksheet.cell(row=1, column=start_col+1).value = 'index_end '+str(m)
        ...
        for j in range(len(array1)):
            worksheet.cell(row=j+2, column=start_col).value = array3[j]
    ...
    meine_liste = np.array(duration_list)
    meine_liste=np.concatenate(meine_liste)
    meine_liste=meine_liste/2
