# -*- coding: utf-8 -*-
"""
Created on Tue May  9 13:42:54 2023
...
# -*- coding: utf-8 -*-
"""
Created on Fri May  5 12:26:03 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 15 15:25:12 2023
...
for start,stop in [0,1200],[1200,2400],[2400,3600]:
    ...
    for m in names:
        ...
        worksheet.cell(row=1, column=start_col).value = 'auc '+str(m)
        worksheet.cell(row=1, column=start_col+1).value = 'norm_auc '+str(m)
        ...
        #data=pd.read_excel(r"C:\Users\wiesbrock\Desktop\Uterus\Uterus\Daten Muscle\circular.xlsx", sheet_name=sheet)
        row_new=data[m]
        row_new[row_new>1.7]=0
        row_new[row_new<-1.7]=0
        #row[row==np.max(row)]=np.nan
        #row[row==np.min(row)]=np.nan
        ...
        xx=np.linspace(0,len(row),len(row))
        for i in range(len(events_start)):
            row_new[row_new<0]=0
            
            amp_low[i]=auc(xx[events_start[i]:events_stop[i]],np.abs(row_new[events_start[i]:events_stop[i]]))
    ...
    workbook.save(r'final_2/V17'+str(sheet)+'_'+str(exp)+'_'+str(start)+'_to_'+str(stop)+".xlsx")
    
    wb = openpyxl.load_workbook(r'final_2/V17'+str(sheet)+'_'+str(exp)+'_'+str(start)+'_to_'+str(stop)+".xlsx")
    n_sheet = workbook['List']
    
    n_sheet.cell(row=1, column=1).value = 'auc '
